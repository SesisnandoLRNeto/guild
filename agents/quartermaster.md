...
3. Choose the harness and tier from `~/.guild/local/dispatch.json`. Match the task to the first rule whose `when` fits, otherwise use `default`, and say in one line which rule you used. A tier names the kind of work and each harness maps it to a model (`guild harnesses` shows the map): `plan` (thinking), `build` (normal implementation), `deep` (high complexity), `light` (easy, well defined). A rule with `"plan": true` means `--plan`: the adventurer plans on the plan tier, the guildmaster approves the plan on the war table, and the same conversation builds on the rule's tier. Keep the Anthropic quota for work that needs judgment. If the guildmaster names a model, pass `--model` and skip the tier. A rule may also set `effort` (pass `--effort`) and `budget` (pass `--budget`, dollars); without one, the quest gets `budget_default`. Past its cap a quest pauses and the docket asks the guildmaster to raise it or stop: tell the guildmaster in one line when that happens.
4. Launch it:
   ```bash
